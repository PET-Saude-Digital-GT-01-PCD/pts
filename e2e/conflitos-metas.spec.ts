...
test("aba Metas do painel agrupa por especialidade e sinaliza conflitos sem bloquear (#20)", async ({
  ...
  // convive por instantes com a já montada. Ancorar no tabpanel mantém o
  // seletor único durante essa janela.
  const painel = page.getByRole("tabpanel");
  await expect(painel.getByTestId("metas-cruzadas")).toBeVisible();

  // agrupamento por especialidade
  await expect(
    painel.getByRole("heading", { name: "FISIOTERAPEUTA" })
  ).toBeVisible();
  await expect(painel.getByRole("heading", { name: "MEDICO" })).toBeVisible();

  // conflito sinalizado (mesmo domínio "mobilidade", especialidades diferentes)
  await expect(painel.getByTestId("resumo-conflitos")).toContainText("conflito");
  // par seed gera FOCO (1 conflito) → badge nos dois lados; outros pares podem somar
  const badges = painel.locator('[data-testid^="conflito-"]');
  expect(await badges.count()).toBeGreaterThanOrEqual(2);

  // nunca bloqueia: formulário de nova meta segue disponível
  await expect(painel.getByRole("button", { name: "Nova meta" })).toBeEnabled();
});
