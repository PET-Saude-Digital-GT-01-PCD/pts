...
/** Recebe uma data civil e devolve a semana de segunda a segunda no fuso de Brasília. */
export function intervaloSemanaAgenda(dataReferencia: string): IntervaloAgenda {
  ...
  const inicio = new Date(segundaCivil.getTime() + OFFSET_BRASILIA_MS);
  return {
    inicio,
...
export function dataCivilBrasilia(data: Date): string {
